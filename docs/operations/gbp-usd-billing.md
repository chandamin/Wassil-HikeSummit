...
7. Set `ENABLE_GBP_USD_BILLING=true`, `ENABLE_RENEWAL_FX_WORKER=true`, and `AIRWALLEX_RENEWAL_SANDBOX_VERIFIED=true` only after the subscription checks. These switches and `AIRWALLEX_BILLING_WEBHOOK_SECRET` are required for production USD checkout and USD BigCommerce orders. Do not use the verification flag to bypass testing.
...
- To stop new production USD payments and orders, unset either `ENABLE_GBP_USD_BILLING` or `ENABLE_RENEWAL_FX_WORKER`. Already-created USD intents are still eligible for verified order reconciliation. This does not cancel or change already-created USD subscriptions; monitor and handle them separately.
